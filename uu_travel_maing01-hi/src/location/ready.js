...
export const Ready = UU5.Common.VisualComponent.create({
  ...
  componentDidMount() {
    if (this.props.detailId) {
      let location = this.props.data.find(item => item.id === this.props.detailId);
      location && this._handleDetail(location);
    }
  },
  //@@viewOff:reactLifeCycle
  //@@viewOn:private
  ...
  _getActions() {
    return [
      {
        content: this.getLsi("create"), // Createlocation Button
        onClick: () => {
          this._modal.open({
